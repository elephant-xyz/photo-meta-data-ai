...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/schema-fixer.log"), logging.StreamHandler()],
)
...
def get_schema_for_file(filename: str, schemas: Dict[str, Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    if "lot" in filename_lower:
        ...
        logger.info(
            f"🔍 Selected lot schema for {filename} (properties: {len(schema.get('properties', {})) if schema else 0})"
        )
        return schema
    elif "layout" in filename_lower:
        schema = schemas.get("layout")
        logger.info(
            f"🔍 Selected layout schema for {filename} (properties: {len(schema.get('properties', {})) if schema else 0})"
        )
        return schema
    elif "structure" in filename_lower:
        schema = schemas.get("structure")
        logger.info(
            f"🔍 Selected structure schema for {filename} (properties: {len(schema.get('properties', {})) if schema else 0})"
        )
        return schema
    elif "utility" in filename_lower:
        schema = schemas.get("utility")
        logger.info(
            f"🔍 Selected utility schema for {filename} (properties: {len(schema.get('properties', {})) if schema else 0})"
        )
        return schema
    elif "appliance" in filename_lower:
        schema = schemas.get("appliance")
        logger.info(
            f"🔍 Selected appliance schema for {filename} (properties: {len(schema.get('properties', {})) if schema else 0})"
        )
        return schema
    elif filename_lower == "property.json":
        schema = schemas.get("property")
        logger.info(
            f"🔍 Selected property schema for {filename} (properties: {len(schema.get('properties', {})) if schema else 0})"
        )
        return schema
    elif filename_lower.startswith("file_") or "photo_metadata" in filename_lower:
        schema = schemas.get("file")
        logger.info(
            f"🔍 Selected file schema for {filename} (properties: {len(schema.get('properties', {})) if schema else 0})"
        )
        return schema
...
def add_missing_required_fields(
    data: Dict[str, Any], schema: Dict[str, Any], field_path: List[str] = None, filename: str = ""
) -> bool:
...
def validate_and_fix_data(data: Dict[str, Any], schema: Dict[str, Any], filename: str, file_path: Path) -> bool:
    ...
    if filename == "bafkreibzrfmqka5h7dnuz7jzilgx4ht5rqcrx3ocl23nger65frbb5hzma.json" or (
    ...
    if not filename.startswith("relationship_"):
        ...
        if "source_http_request" not in data:
            ...
            modified = True
            logger.info("  ➕ Added source_http_request")
    ...
    if any(keyword in filename for keyword in ["appliance", "lot", "structure", "utility"]) and not filename.startswith(
        "relationship_"
    ):
        request_id_value = property_id if property_id else "N/A"
        if "request_identifier" not in data:
    ...
    if filename.startswith("file_") and "photo_metadata" in filename and not filename.startswith("relationship_"):
        if "document_type" not in data or data["document_type"] != "PropertyImage":
            ...
            modified = True
            logger.info("  🔧 Set document_type to PropertyImage")
        ...
        if "file_format" not in data or data["file_format"] != "jpeg":
            ...
            modified = True
            logger.info("  🔧 Set file_format to jpeg")
    ...
    if "layout" in filename and not filename.startswith("relationship_"):
        if "source_http_request" not in data:
            data["source_http_request"] = {"method": "GET", "url": "https://pbcpao.gov/Property/Details"}
            modified = True
            logger.info("  ➕ Added source_http_request to layout file")
                    ...
                    # Check for boolean type - enhanced logic from fix_and_submit_local.py
                        ...
                        if isinstance(value, str):
                            if value.lower() in ["true", "yes", "present", "available"]:
                                ...
                                obj[key] = False
                                logger.info(
                                    f"  🔧 Fixed invalid boolean field {'.'.join(current_path)}: {value} -> false"
                                )
                                modified = True
                        elif value is None:
