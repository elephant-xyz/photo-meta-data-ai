...
class ImageConverter:
    ...
    def convert_to_webp(
        self, image_input: Union[str, bytes, Image.Image], quality: int = DEFAULT_WEBP_QUALITY, lossless: bool = False
    ) -> bytes:
        """
        ...
        try:
            ...
            if isinstance(image_input, str):
                ...
                self.logger.info(
                    f"Converted to WebP: {original_size} -> {webp_size} bytes " f"({compression_ratio:.1f}% reduction)"
                )
    ...
    def optimize_for_upload(
        self, image_path: str, max_size_mb: float = 5.0, target_format: str = "webp"
    ) -> Tuple[bytes, str]:
        """
    ...
    def batch_convert_to_webp(
        self, image_paths: list, output_dir: str, quality: int = DEFAULT_WEBP_QUALITY, preserve_names: bool = True
    ) -> list:
        """
...
def convert_to_webp(image_input: Union[str, bytes, Image.Image], quality: int = 85, lossless: bool = False) -> bytes:
    """Convert an image to WebP format."""
...
def convert_to_webp_base64(
    image_input: Union[str, bytes, Image.Image], quality: int = 85, lossless: bool = False
) -> str:
    """Convert an image to WebP format and return as base64 string."""
    converter = ImageConverter()
...
def optimize_image_for_upload(image_path: str, max_size_mb: float = 5.0) -> Tuple[bytes, str]:
