...
class RekognitionCategorizer:
    ...
    def analyze_image_with_rekognition(self, image_key):
        ...
        try:
            response = self.rekognition_client.detect_labels(
                Image={"S3Object": {"Bucket": self.bucket_name, "Name": image_key}}, MaxLabels=10, MinConfidence=70
            )
    ...
    def copy_image_to_category(self, source_key, property_id, category, image_name):
        ...
        try:
            ...
            self.s3_client.copy_object(
                Bucket=self.bucket_name, CopySource={"Bucket": self.bucket_name, "Key": source_key}, Key=destination_key
            )
        ...
        try:
            ...
            for obj in response["Contents"]:
                ...
                if len(key_parts) >= 3:
                    ...
                    try:
                        self.s3_client.download_file(self.bucket_name, s3_key, local_image_path)
                        downloaded_count += 1
    ...
    def print_comprehensive_summary(self, all_results):
        ...
        print(
            f"📈 SUCCESS RATE: {(total_categorized / total_images * 100):.1f}%"
            if total_images > 0
        ...
        for property_id, result in all_results.items():
            ...
            print(
                f"   Success Rate: {(result['categorized_images'] / result['total_images'] * 100):.1f}%"
        ...
        try:
            ...
            if labels:
                ...
                if self.copy_image_to_category(image_key, property_id, category, image_name):
                    ...
                    return {"success": True, "image_detail": image_detail, "category": category}
                else:
                    return {"success": False, "error": f"Failed to copy {image_name} to category folder"}
            else:
                return {"success": False, "error": f"No labels detected for {image_name}"}

        except Exception as e:
            return {"success": False, "error": f"Error processing {image_key}: {str(e)}"}
...
def main():
    ...
    try:
        response = categorizer.s3_client.list_objects_v2(Bucket=bucket_name, Delimiter="/")
