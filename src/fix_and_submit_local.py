...
def get_parcel_id_from_property_json(folder_path):
    ...
    try:
        ...
        parcel_id = data.get("parcel_identifier") or data.get("request_identifier") or data.get("parcelId")
...
def fix_source_http_request_in_file(file_path, property_id=None):
    ...
    try:
        ...
        if filename == "bafkreih226p5vjhx33jwgq7trblyplfw7yhkununuuahgpfok3hnh5mjwq.json" or (
            filename.endswith(".json") and filename.startswith("bafkre")
        ):
        ...
        if not filename.startswith("relationship_"):
            ...
            if "source_http_request" not in data:
                data["source_http_request"] = {"method": "GET", "url": "https://pbcpao.gov/Property/Details"}
                modified = True
                print("  ➕ Added source_http_request")
        ...
        if (
            property_id
            and any(keyword in filename for keyword in ["appliance", "lot", "structure", "utility"])
            and not filename.startswith("relationship_")
        ):
            if "request_identifier" not in data:
                data["request_identifier"] = property_id
                modified = True
        ...
        if "file_" in filename and "photo_metadata" in filename and not filename.startswith("relationship_"):
            if "document_type" not in data or data["document_type"] != "PropertyImage":
                ...
                modified = True
                print("  🔧 Set document_type to PropertyImage")
        ...
        if "layout" in filename and not filename.startswith("relationship_"):
            if "source_http_request" not in data:
                data["source_http_request"] = {"method": "GET", "url": "https://pbcpao.gov/Property/Details"}
                modified = True
                print("  ➕ Added source_http_request to layout file")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if (
                ...
                or "http" in stderr
                and "timeout" in stderr
                or "504" in stderr
                or "status: 504" in stderr
            ):
        ...
        with open(csv_file, "r") as f:
            ...
            for row in reader:
                if row.get("status", "").lower() == "error":
                    errors.append({"file": row.get("file", ""), "error": row.get("error", ""), "details": row})
    except Exception as e:
    ...
    try:
        with open(submit_errors_file, "r") as f:
            ...
            for row in reader:
                ...
                print(
                    f"📋 Parsed error from CSV: file='{error_data['file']}', error='{error_data['error']}', path='{error_data['path']}'"
                )
    ...
    for error in errors:
        ...
        if error_path and "/relationships/" in error_path:
            ...
            if actual_data_file:
                ...
                print("❌ Failed to trace relationship to target file")
        ...
        try:
            ...
            is_relationship_file = (
                "relationship" in filename
                or filename.startswith("bafkreicaq62gggwbppihgstao2maakafmghjttf73ai53tz5tam2cixrvu")
                or (isinstance(data, dict) and "relationships" in data and "label" in data)
            )
            if schema:
            ...
            if (
                ...
                and "photo_metadata" in filename
                and not filename.startswith("relationship_")
            ):
                if "document_type" not in data or data["document_type"] != "PropertyImage":
                    data["document_type"] = "PropertyImage"
            ...
            if "must be one of:" in error_msg:
                print("🔧 Found 'must be one of' error, attempting to fix...")
            ...
            # Auto-fix "must be string" errors by setting value to empty string or property_id
                ...
                print("🔧 Found 'must be string' error, attempting to fix...")
    ...
    for root, dirs, files in os.walk(submit_dir):
        for file in files:
            if file.endswith(".json") and "layout" in file.lower():
                ...
                if file == "bafkreibzrfmqka5h7dnuz7jzilgx4ht5rqcrx3ocl23nger65frbb5hzma.json" or file.startswith(
                    "relationship_"
                ):
                    continue
    ...
    for root, dirs, files in os.walk(submit_dir):
        for file in files:
            if file.endswith(".json"):
                ...
                is_target_file = any(
                    keyword in file.lower() for keyword in ["appliance", "lot", "structure", "utility"]
                )
                ...
                if is_target_file and not file.startswith("relationship_"):
                    ...
                    try:
                        ...
                        # Ensure request_identifier is present with parcel_id
                        if property_id and (
                            "request_identifier" not in data or data["request_identifier"] != property_id
                        ):
                            data["request_identifier"] = property_id
                            modified = True
                            print(f"  🔧 Updated request_identifier to {property_id} in {file}")
...
def fix_errors_with_openai(submit_dir, errors):
    ...
    for i in range(0, len(errors), batch_size):
        ...
        print(
            f"🤖 Processing batch {i // batch_size + 1}/{(len(errors) + batch_size - 1) // batch_size} ({len(batch)} errors)"
        )
...
"""
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a JSON validation expert specializing in Elephant Network schemas. Always return valid JSON that matches the provided schema requirements.",
                        },
                ...
                try:
                    ...
                    is_relationship_file = (
                        "relationship" in filename
                        or filename.startswith("bafkreicaq62gggwbppihgstao2maakafmghjttf73ai53tz5tam2cixrvu")
                        or (isinstance(fixed_data, dict) and "relationships" in fixed_data and "label" in fixed_data)
                    )
                    schema = get_schema_for_file(target_file)
                    if schema:
                        fixed_data, schema_modified = ensure_all_schema_properties(
                            fixed_data, schema, is_relationship_file
                        )
                        if schema_modified:
                            print("    ➕ Added missing schema properties")
